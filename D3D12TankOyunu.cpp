...
vector<intersect> intersections2;
XMMATRIX g_World;
XMMATRIX g_View;
...
XMFLOAT4 initial_Enemy_Missile_Position_Float4;
...
void OnUpdate()
{
	...
	{
		// tankın duvardan geçmesini engelle
		XMVECTOR Ro = Eye;
	...
	if (FireEnemyMissile)
	{
		Ro_Enemy_Missile = XMVectorSet(0, 1, 2.5, 0);
		Rd_Enemy_Missile = XMVector3Normalize(Ro_Tank_Missile - Ro_Enemy_Missile);
		XMVECTOR initialPosition = Ro_Enemy_Missile;		// Namlunun ucu Tankın merkezinden 2.5 birim ileride
		XMFLOAT4 initialPosition_F4; XMStoreFloat4(&initialPosition_F4, initialPosition);
	...
	XMFLOAT4X4 g_World_Missile2_4x4;

	// Kesişim Ayarları
	intersections2 = testIntersections(Ro_Enemy_Missile, Rd_Enemy_Missile, g_World_Tank);

	if (intersections2.size() > 0)
	{
		intersect nearest = nearestObject(intersections2);
		...
		float Missile_Tank_Distance = XMVectorGetX(XMVector3Length(Ro_Tank_Missile - Missile_Position));
		if ((nearest.isEnemy) && Missile_Tank_Distance < 1.0f)
		{
			PlaySound(TEXT("hit.mav"), NULL, SND_FILENAME | SND_ASYNC);
			renderTank = false;
			renderTankMissile = false;
